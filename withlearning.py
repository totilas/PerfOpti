...
for j, level in enumerate(levels):
    ...
    learner_groups = [
        [RPPerfGD2(Pi=np.zeros((7,7)), step_size=.1, Pi_learning=True) for _ in range(num_iter_average)],
        [SFPerfGD2(Pi=np.zeros((7,7)), step_size=.1, Pi_learning=True, sigma=scale_0 * level) for _ in range(num_iter_average)],
        [PiRGD2(Pi=np.zeros((7,7)), step_size=.1, Pi_learning=True) for _ in range(num_iter_average)],
        [PiRGD2(Pi=np.zeros((7,7)), step_size=.1, Pi_learning=True, lamb=1e-1) for _ in range(num_iter_average)]
    ]
...
def plot_norms(histories, fig, ax, l, cma, co, pi_perf):
    norms_history = []
    norm_pi_perf = np.linalg.norm(pi_perf, 'fro')

    for Pi_history in histories:
        norms = []
        for Pi in Pi_history:
            try:
                norm = np.linalg.norm(Pi - pi_perf, 'fro')
                if np.isnan(norm) or np.isinf(norm):
                    norms.append(norm_pi_perf)
                else:
                    norms.append(norm)
            except:
                norms.append(norm_pi_perf)  # Handle any unexpected errors gracefully
        norms_history.append(np.array(norms))

    norms_history = np.stack(norms_history, axis=1)
    
    # Compute mean and std, ignoring NaNs
    norms_history_mean = np.nanmean(norms_history, axis=1)
    norms_history_std = np.nanstd(norms_history, axis=1)

    ax.errorbar(
        np.arange(len(norms_history_mean)),
        norms_history_mean,
        norms_history_std,
        marker=cma,
        linestyle='-',
    ...
    ax.set_ylim([0, np.nanmax(norms_history_mean) * 1.1])
    ax.set_xlabel('Iterations', fontsize=14)
    ax.set_ylabel('Frobenius Norm of Difference', fontsize=14)
    ax.legend()
...
for j, level in enumerate(levels):
    for i, group in enumerate(all_groups[j]):
        print(type(group[0]).__name__+ str(level))
...
for j, level in enumerate(levels):
    for i, group in enumerate(all_groups[j]):
        learner_name = type(group[0]).__name__ + str(level)
        plot_norms(
            [learner.Pis_history() for learner in group],
            fig,
            ax,
            learner_name,
            cmas[j],
...
# Add legends
first_legend = plt.legend(handles=marker_handles, loc='upper left', fontsize=14)
ax.add_artist(first_legend)
plt.legend(handles=color_handles,  loc='upper right', fontsize=14)

plt.savefig("normslearning_.pdf")
