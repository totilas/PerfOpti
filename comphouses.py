...
cos = ["Blue", "Green",'xkcd:sky', "Yellow", "Purple", "Orange", "Pink", "Gray", "Brown"]
cmas = ["Reds", "Blues", "Greens", "YlOrBr", "Purples", "Oranges", "Pink", "cool", "Greys", "YlOrBr"]

mymarkers=['^', 'o', 'x']
level_markers = {'^': "$\lambda = 0$", 'o': '$\lambda = 1$', 'x': '$\lambda = 10$'}
algorithm_colors = {'Blue': 'RPPerfGD', 'Green': 'RGD', 'xkcd:sky':'RRGD'}
...
def plot_traj(history, fig, ax, l, cma, co, subplot_idx, stddev):
    ...
    ax_dist.errorbar(range(len(distances)), distances, yerr=distance_std, ms=5, linestyle='-', alpha=0.7, color=co, label=l, marker=cma)
...
for k, eps in enumerate(epss):
    ...
    for l,learner_class in enumerate([RPPerfGD,RGD,PiRGD]):
        accuracies = []
        theta_histories = []

        for run in range(n_runs):
            learner = learner_class(Pi=Pi, step_size=.2, lamb =5e-3) if learner_class==PiRGD else learner_class(Pi=Pi, step_size=.2)
